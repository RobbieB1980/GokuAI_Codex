...
$prompt="LOCAL-FIRST REPAIR. You are KAT running locally through the Codex tool host; do not claim to be an OpenAI model. Open and follow the complete repair request at: $PromptFile. Repair only: $failed. Read hardened fixes and exact-version evidence before editing. Build and iterate. If evidence is conflicting or the repair remains unresolved after bounded attempts, stop and write KAT_ESCALATION.md; do not call an online model."
$codexArgs=@('-C',$failed,'--add-dir',$workspace,'-s','workspace-write','-a','on-request','-m','goku-local','-c','model_provider="goku"','-c','model_providers.goku.name="Local KAT"','-c',"model_providers.goku.base_url=`"http://127.0.0.1:$Port/v1`"",'-c','model_providers.goku.wire_api="responses"',$prompt)

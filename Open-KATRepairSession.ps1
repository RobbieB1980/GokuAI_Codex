...
$codexArgs=@('-C',$failed,'--add-dir',$workspace,'-s','workspace-write','-a','never','-m','goku-local','-c','model_provider="goku"','-c','model_providers.goku.name="Local KAT"','-c',"model_providers.goku.base_url=`"http://127.0.0.1:$Port/v1`"",'-c','model_providers.goku.wire_api="responses"','-c','mcp_servers.codex_apps.enabled=false','-c','model_context_window=65536','-c','model_auto_compact_token_limit=48000',$prompt)
